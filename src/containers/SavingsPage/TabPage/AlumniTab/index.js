...
class AlumniTab extends Component {
	onConfirm = (selected) => {
		const keywords = [];
					...
					<Content>
						<ScrollView>
							...
							<View style={styles.scrollWrapper}>
								{alumni_offers && alumni_offers.length && !loading ?
									alumni_offers.map((alumni, index) => {
										return (
											<View
												key={index}
												style={{ width: '50%', height: 161, borderRadius: 5, paddingLeft: 8, paddingRight: 8, marginBottom: 22 }}
											>
												<TouchableOpacity onPress={() => Actions.offersDetailPage({ alumniOfferDetail: alumni, title: 'Alumni Offers' })}>
													<Card style={{ borderRadius: 5 }}>
														...
														>
															<Image
																source={{ uri: `https://alum.ust.hk/${alumni.thumbnail}` }} // Temp Image
																style={styles.image}
															/>
														</CardItem>
														<CardItem
